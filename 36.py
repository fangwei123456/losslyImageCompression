...
class EncodeNet(nn.Module):
    def __init__(self):
        ...
        self.conv3_8 = nn.Conv2d(32, 32, 5, padding=2)
        self.conv3_9 = nn.Conv2d(32, 32, 5, padding=2)
        self.conv3_10 = nn.Conv2d(32, 32, 5, padding=2)
        self.conv3_11 = nn.Conv2d(32, 32, 5, padding=2)
        self.conv3_12 = nn.Conv2d(32, 32, 5, padding=2)
        self.conv3_13 = nn.Conv2d(32, 32, 5, padding=2)
        self.conv3_14 = nn.Conv2d(32, 32, 5, padding=2)
        self.conv3_15 = nn.Conv2d(32, 32, 5, padding=2)

        self.conv_down_0 = nn.Conv2d(128, 128, 2, 2) # 降采样
        self.conv_down_1 = nn.Conv2d(32, 32, 2, 2)  # 降采样
        ...
        self.bn32_8 = nn.BatchNorm2d(32)
        self.bn32_9 = nn.BatchNorm2d(32)
        self.bn32_10 = nn.BatchNorm2d(32)
        self.bn32_11 = nn.BatchNorm2d(32)
        self.bn32_12 = nn.BatchNorm2d(32)
        self.bn32_13 = nn.BatchNorm2d(32)
        self.bn32_14 = nn.BatchNorm2d(32)
        self.bn32_15 = nn.BatchNorm2d(32)
        ...
        self.bn32_A_4 = nn.BatchNorm2d(32)
        self.bn32_A_5 = nn.BatchNorm2d(32)
        self.bn32_A_6 = nn.BatchNorm2d(32)
        self.bn32_A_7 = nn.BatchNorm2d(32)
    ...
    def forward(self, x):
        ...
        # n*32*128*128 -> n*32*64*64
        x = self.conv_down_1(x)

        xA = self.bn32_A_4(x)

        x = F.leaky_relu(self.conv3_8(x))
        x = x / (torch.norm(x) + 1e-6)
        x = self.bn32_8(x)

        x = F.leaky_relu(self.conv3_9(x))
        x = x / (torch.norm(x) + 1e-6)
        x = self.bn32_9(x)
        ...
        x = F.leaky_relu(self.conv3_10(x))
        x = x / (torch.norm(x) + 1e-6)
        x = self.bn32_10(x)

        x = F.leaky_relu(self.conv3_11(x))
        x = x / (torch.norm(x) + 1e-6)
        x = self.bn32_11(x)
        ...
        x = F.leaky_relu(self.conv3_12(x))
        x = x / (torch.norm(x) + 1e-6)
        x = self.bn32_12(x)

        x = F.leaky_relu(self.conv3_13(x))
        x = x / (torch.norm(x) + 1e-6)
        x = self.bn32_13(x)
        ...
        x = F.leaky_relu(self.conv3_14(x))
        x = x / (torch.norm(x) + 1e-6)
        x = self.bn32_14(x)

        x = F.leaky_relu(self.conv3_15(x))
        x = x / (torch.norm(x) + 1e-6)
        x = self.bn32_15(x)
...
class DecodeNet(nn.Module):
    def __init__(self):
        ...
        self.tconv3_8 = nn.ConvTranspose2d(32, 32, 5, padding=2)
        self.tconv3_9 = nn.ConvTranspose2d(32, 32, 5, padding=2)
        self.tconv3_10 = nn.ConvTranspose2d(32, 32, 5, padding=2)
        self.tconv3_11 = nn.ConvTranspose2d(32, 32, 5, padding=2)
        self.tconv3_12 = nn.ConvTranspose2d(32, 32, 5, padding=2)
        self.tconv3_13 = nn.ConvTranspose2d(32, 32, 5, padding=2)
        self.tconv3_14 = nn.ConvTranspose2d(32, 32, 5, padding=2)
        self.tconv3_15 = nn.ConvTranspose2d(32, 32, 5, padding=2)

        self.tconv_up_0 = nn.ConvTranspose2d(128, 128, 2, 2) # 升采样
        self.tconv_up_1 = nn.ConvTranspose2d(32, 32, 2, 2)  # 升采样
    ...
    def forward(self, x):
        ...
        x = F.leaky_relu(self.tconv3_8(x))
        x = x * (torch.norm(x) + 1e-6)
        x = self.bn32_8(x)

        x = F.leaky_relu(self.tconv3_9(x))
        x = x * (torch.norm(x) + 1e-6)
        x = self.bn32_9(x)
        ...
        x = F.leaky_relu(self.tconv3_10(x))
        x = x * (torch.norm(x) + 1e-6)
        x = self.bn32_10(x)

        x = F.leaky_relu(self.tconv3_11(x))
        x = x * (torch.norm(x) + 1e-6)
        x = self.bn32_11(x)
        ...
        x = F.leaky_relu(self.tconv3_12(x))
        x = x * (torch.norm(x) + 1e-6)
        x = self.bn32_12(x)

        x = F.leaky_relu(self.tconv3_13(x))
        x = x * (torch.norm(x) + 1e-6)
        x = self.bn32_13(x)
        ...
        x = F.leaky_relu(self.tconv3_14(x))
        x = x * (torch.norm(x) + 1e-6)
        x = self.bn32_14(x)

        x = F.leaky_relu(self.tconv3_15(x))
        x = x * (torch.norm(x) + 1e-6)
        x = self.bn32_15(x)
