...
class EncodeNet_64_32_128_128(nn.Module): # 128*256*256 -> 128*128*128
    def __init__(self):
...
optimizer = torch.optim.Adam([{'params':encodeNet3.parameters()},{'params':decodeNet3.parameters()}], lr=float(sys.argv[3]))
batchSize = 6
trainData = torch.empty([batchSize, 64, 128, 128]).float().cuda()
imgNum = 128
for i in range(int(sys.argv[4])):

    if(i%100==0):
        imgData = torch.randn([imgNum, 64, 128, 128]) * 10  # 使用随机生成的数据预训练
    ...
    while(1):
        ...
        optimizer.zero_grad()
        output = decodeNet3(encodeNet3(trainData))
    ...
    if (i == 0):
        ...
        if (maxLossOfTrainData < minLoss):  # 保存最小loss对应的模型
            minLoss = maxLossOfTrainData
            torch.save(encodeNet3, './models/encodeNet3_' + sys.argv[5] + '.pkl')
            print('./models/encodeNet3_' + sys.argv[5] + '.pkl')
            torch.save(decodeNet3, './models/decodeNet3_' + sys.argv[5] + '.pkl')
            print('./models/decodeNet3_' + sys.argv[5] + '.pkl')
