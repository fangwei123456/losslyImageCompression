...
class NormNet(nn.Module):
    ...
    def forward(self, input):
        output = self.conv(input.pow(2)).sqrt()
        output = input / output
        return output
...
class EncodeNet(nn.Module):
    def __init__(self):
        ...
        self.norm0 = NormNet(128)
        self.norm1 = NormNet(128)
...
class DecodeNet(nn.Module):
    def __init__(self):
        ...
        self.tnorm0 = TNormNet(128)
        self.tnorm1 = TNormNet(128)
...
if(sys.argv[2]=='0'): # 设置是重新开始 还是继续训练
    ...
    torch.nn.init.constant_(net.enc.norm0.conv.weight, 1)
    torch.nn.init.constant_(net.enc.norm0.conv.bias, 0)

    torch.nn.init.constant_(net.enc.norm1.conv.weight, 1)
    torch.nn.init.constant_(net.enc.norm1.conv.bias, 0)

    torch.nn.init.constant_(net.dec.tnorm0.conv.weight, 1)
    torch.nn.init.constant_(net.dec.tnorm0.conv.bias, 0)

    torch.nn.init.constant_(net.dec.tnorm1.conv.weight, 1)
    torch.nn.init.constant_(net.dec.tnorm1.conv.bias, 0)
