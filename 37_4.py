...
optimizer = torch.optim.Adam([{'params':encodeNet4.parameters()},{'params':decodeNet4.parameters()}], lr=float(sys.argv[3]))
batchSize = 6
trainData = torch.empty([batchSize, 32, 128, 128]).float().cuda()
imgNum = 128
for i in range(int(sys.argv[4])):

    if(i%100==0):
        imgData = torch.randn([imgNum, 32, 128, 128]) * 10  # 使用随机生成的数据预训练
    ...
    while(1):
        ...
        optimizer.zero_grad()
        output = decodeNet4(encodeNet4(trainData))
    ...
    if (i == 0):
        ...
        if (maxLossOfTrainData < minLoss):  # 保存最小loss对应的模型
            minLoss = maxLossOfTrainData
            torch.save(encodeNet4, './models/encodeNet4_' + sys.argv[5] + '.pkl')
            print('./models/encodeNet4_' + sys.argv[5] + '.pkl')
            torch.save(decodeNet4, './models/decodeNet4_' + sys.argv[5] + '.pkl')
            print('./models/decodeNet4_' + sys.argv[5] + '.pkl')
